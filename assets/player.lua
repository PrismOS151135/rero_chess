...
function Player.new(id, data, game)
    ...
    assertf(#player.dice.points == #player.dice.weights, "Dice points and weights mismatch (%d vs %d)",
        #player.dice.points, #player.dice.weights)
...
local function moveThread(self, stepCount, manual)
    ...
    while math.abs(self.stepRemain) >= 1 do
        local sx, sy = self.x, self.y
        local ex, ey = self.game.map[self.nextLocation].x + MATH.rand(-.15, .15),
            self.game.map[self.nextLocation].y + MATH.rand(-.15, .15)
        local animLock = true
...
function Player:draw()
    ...
    while true do
        -- Layer 1
        do
            if coroutine.yield() ~= true then error("Rendering signal mis-match") end
            -- Color Mark
        ...
        do
            ...
            if self.canBeSelected then
                gc_setColor(1, 1, 1)
                gc_mDrawQ(TEX.world.default, QUAD.world.target, 0, -0.3, love.timer.getTime() + self.id,
                    math.max(.005, ((.5 / self.game.cam.k) / .005) ^ .8 * .005) *
                    (1 + .26 * math.sin(love.timer.getTime() * 6.26)))
                -- gc_setColor(0,0,1)
